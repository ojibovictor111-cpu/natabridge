...
import type { StoredPredictionOptions } from "../../models/prediction/service/prediction.service";
...
const isAiApiResponse = (value: unknown): value is AiApiResponse => {
	...
	const validFactors =
		Array.isArray(value.topFactors) &&
		value.topFactors.every(
			(factor) =>
				isRecord(factor) &&
	...
	return (
		["Low Risk", "Mid Risk", "High Risk"].includes(String(value.risk)) &&
...
const getAiPrediction = async (
	...
	try {
		const response = await fetch(`${process.env.ai_origin}/predict`, {
			method: "POST",
			headers: {
		...
		if (!response.ok) {
			throw new ClientFacingError({
				...
				message:
					"The risk prediction service could not complete the prediction. Please try again shortly.",
				cause: new Error(
					`Prediction service responded with status ${response.status}`,
				),
			});
		}
		...
		try {
			...
			throw new ClientFacingError({
				...
				message:
					"The risk prediction service returned an invalid response. Please try again shortly.",
				cause: error,
			});
		}
		...
		throw new ClientFacingError({
			...
			message:
				"The risk prediction service is temporarily unavailable. Please try again shortly.",
			cause: error,
		});
	}
};
...
const recordPredictionFailure = async (
	...
	await withTransaction(server, (client) =>
		failPredictionRun(client, predictionRunId, failureCode),
	);
};
...
const executeStoredPrediction = async (
	...
	try {
		...
		try {
			...
			throw new ClientFacingError({
				...
				message:
					"The prediction attempt could not be recorded. Please try again.",
	...
	const assessmentId =
		options.assessment === undefined ? undefined : `ass-${uuidv7()}`;
