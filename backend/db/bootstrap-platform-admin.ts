...
const assignFirstPlatformAdmin = async (
    ...
    if (role.rows.length !== 1) {
        throw new Error("Platform admin role is missing. Run npm run db:migrate first.");
    }
    ...
    if (assignments.rows.length > 0) {
        const existing = assignments.rows.find((row) => row.firebase_uid === identity.uid);
        if (existing && assignments.rows.length === 1) {
            if (existing.status !== "ACTIVE" || existing.email.toLowerCase() !== identity.email.toLowerCase()) {
                throw new Error("The existing platform admin account needs manual reconciliation.");
            }
            return { id: existing.id, created: false };
        }
        throw new Error("A platform admin is already assigned. The bootstrap command cannot grant another one.");
    }
    ...
    if (users.rows.some((row) => row.firebase_uid !== identity.uid || row.email.toLowerCase() !== identity.email.toLowerCase())) {
        throw new Error("A user already has this UID or email with different identity details. Reconcile that row manually.");
    }
    ...
    if (!existingUser) {
        await client.query(
            `INSERT INTO users (id, firebase_uid, firstname, lastname, email)
             VALUES ($1, $2, $3, $4, $5)`,
            [id, identity.uid, identity.firstName, identity.lastName, identity.email]
        );
    }
...
const main = async () => {
    ...
    const firebaseUser = await getAuth(getFirebaseApp()).getUser(uid);
    if (firebaseUser.disabled || !firebaseUser.email || !firebaseUser.emailVerified) {
        throw new Error("The Firebase user must be enabled and have a verified email address.");
    }
    if (firebaseUser.email.length > 255) {
        throw new Error("The Firebase user's email address exceeds the database limit.");
    }

    const server = buildServer({ logger: false });
    try {
        await server.ready();
        const result = await withTransaction(server, (client) => assignFirstPlatformAdmin(client, {
            uid,
